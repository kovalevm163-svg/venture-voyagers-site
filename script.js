...
function restoreCriticalOwnerSunriseCredentials() {
  ["aleks.sunrise@vvs.com", "mikhail.sunrise@vvs.com"].forEach((key) => {
...
restoreCriticalOwnerSunriseCredentials();
...
function syncCredentialFieldAcrossLinkedAccounts(rawKey = "", fieldName = "", value = "") {
  ...
  const sourceAccount = sourceKey ? accounts[sourceKey] : null;
  const sourceIsSunriseCredential = !!sourceAccount?.sunriseCredential;
  const keys = relatedAccountKeysForDelete(rawKey);
  keys.forEach((key) => {
    const account = accounts[key];
    if (!account) return;
    if (fieldName === "password" && isProtectedOwnerSunriseCredentialKey(key) && !sourceIsSunriseCredential) {
      return;
    }
